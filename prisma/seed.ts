...
If you cannot help with a request, politely ask the customer to wait for a human agent.`,
  ...
  const conv1LastMessage = new Date(Date.now() - 40 * 60 * 1000);
  const conv2LastMessage = new Date(Date.now() - 15 * 60 * 1000);
  
  await prisma.conversation.update({
    where: { id: conversation1.id },
    data: { lastMessageAt: conv1LastMessage },
  });

  await prisma.conversation.update({
    where: { id: conversation2.id },
    data: { lastMessageAt: conv2LastMessage },
  });

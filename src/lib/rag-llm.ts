...
export class RAGLLMService {
  ...
  static async generateResponse(
    ...
    try {
      // 1. Get company's OpenAI API key if they have OpenAI configured
      ...
      try {
        ...
        if (providerConfig && providerConfig.provider === "OPENAI") {
          ...
          console.log(`🔑 RAG LLM: Using company's OpenAI API key for embeddings`);
        } else {
          console.log(`🔑 RAG LLM: Using global OpenAI API key for embeddings (company provider: ${providerConfig?.provider || 'none'})`);
        }
      } catch (error) {
        console.log(`⚠️ RAG LLM: Could not retrieve company OpenAI key, using global key:`, error);
      }

      // 2. Generate query embedding with company's key if available
...
I don't have specific information to answer that question right now. Is there something else I can help you with?`;
  ...
  static async searchDocuments(
    ...
    // Get company's OpenAI API key if available
    let openaiApiKey: string | undefined;
    try {
      const providerConfig = await db.providerConfig.findUnique({
        where: { companyId },
      ...
      if (providerConfig && providerConfig.provider === "OPENAI") {
        const { decrypt } = await import("./encryption");
        openaiApiKey = await decrypt(providerConfig.apiKeyEnc);
        console.log(`🔑 RAG Search: Using company's OpenAI API key for embeddings`);
      } else {
        console.log(`🔑 RAG Search: Using global OpenAI API key for embeddings (company provider: ${providerConfig?.provider || 'none'})`);
      }
    } catch (error) {
      console.log(`⚠️ RAG Search: Could not retrieve company OpenAI key:`, error);
    }

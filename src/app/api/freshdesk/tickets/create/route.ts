...
export async function POST(request: NextRequest) {
  try {
    ...
    if (includeConversationHistory && conversation.messages.length > 0) {
      ...
      const pagePlatformInfo = conversation.platform === 'INSTAGRAM' 
        ? `Instagram Connection: @${conversation.instagramConnection?.username}`
        : `Facebook Page: ${conversation.pageConnection?.pageName} (${conversation.pageConnection?.pageId})`;

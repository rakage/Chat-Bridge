...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Check if we're in development mode and webhook is not set up
      const isDevMode = process.env.NODE_ENV === "development";
      const skipWebhookCall = process.env.SKIP_WEBHOOK_CALLS === "true";
      
      if (skipWebhookCall && isDevMode) {
        console.log(`🔧 Development mode: Skipping actual webhook ${subscribe ? 'subscription' : 'unsubscription'}`);
      } else {
        if (subscribe) {

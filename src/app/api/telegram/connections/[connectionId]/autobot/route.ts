...
export async function PATCH(
  ...
  try {
    ...
    const updatedConnection = await db.telegramConnection.update({
      where: { id: connectionId },
      data: { autoBot },
    });

    console.log(
      `✅ Updated autoBot for Telegram bot @${updatedConnection.botUsername} (${connectionId}) to ${autoBot}`
    );

    return NextResponse.json({
      success: true,
      autoBot: updatedConnection.autoBot,
      message: `Auto-bot ${autoBot ? "enabled" : "disabled"} for @${updatedConnection.botUsername}`,
    });
  } catch (error) {
    console.error("Error toggling Telegram autoBot:", error);
    return NextResponse.json(

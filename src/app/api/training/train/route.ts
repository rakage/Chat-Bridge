...
async function processTrainingAsync(
  ...
  try {
    ...
    // Supabase vector storage is optional (embeddings are already in PostgreSQL via Prisma)
    if (vectorDocuments.length > 0 && process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      console.log(
        `📦 Storing ${vectorDocuments.length} embeddings in Supabase for faster vector search...`
      );
      ...
      try {
        ...
        console.log("ℹ️  Supabase not configured, using PostgreSQL for vector search");
        // Don't fail the training - PostgreSQL fallback will be used for search
      }
    } else {
      console.log("ℹ️  Using PostgreSQL for vector storage and search (Supabase not configured)");
    }

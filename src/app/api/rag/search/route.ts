  ...
  try {
    ...
    try {
      ...
      if (providerConfig && providerConfig.provider === "OPENAI") {
        ...
        console.log(`🔑 API Search Route: Using company's OpenAI API key for embeddings`);
      } else {
        console.log(`🔑 API Search Route: Using global OpenAI API key for embeddings (company provider: ${providerConfig?.provider || 'none'})`);
      }
    } catch (error) {
      console.log(`⚠️ API Search Route: Could not retrieve company OpenAI key:`, error);
    }

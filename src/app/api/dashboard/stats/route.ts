...
const CACHE_TTL = 300;
const CACHE_KEY_PREFIX = "dashboard:stats:";
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (cached) {
        console.log(`📊 Dashboard stats cache HIT for company: ${companyId}`);
    ...
    // ============================================
    // STEP 2: Store in Redis cache for future requests
    // ============================================
    try {
      await redis.setex(cacheKey, CACHE_TTL, JSON.stringify(stats));
      console.log(`💾 Dashboard stats cached for company: ${companyId} (TTL: ${CACHE_TTL}s)`);
    } catch (cacheError) {

...
export async function GET(request: NextRequest) {
  try {
    ...
    // Max 50 requests per minute, block for 5 minutes
    ...
    // Process conversation trends (count unique conversations per day from messages, grouped by platform)
    ...
    allMessages.forEach((msg) => {
      ...
      const platform = msg.conversation.platform;
      conversationsByDate[date].total.add(msg.conversationId);
    ...
    const conversationTrends = Object.entries(conversationsByDate).map(
      ([date, platforms]) => ({ 
        date, 
        facebook: platforms.facebook.size,
        instagram: platforms.instagram.size,
        telegram: platforms.telegram.size,
        widget: platforms.widget.size,
        total: platforms.total.size,
      })
    );
    ...
    for (let i = 0; i <= 30; i++) {
      ...
      conversationData.push({
        date: displayDate,
        facebook: existingData?.facebook || 0,
        instagram: existingData?.instagram || 0,
        telegram: existingData?.telegram || 0,
        widget: existingData?.widget || 0,
